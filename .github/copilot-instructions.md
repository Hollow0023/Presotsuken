...
## Git・PR作成ガイドライン
- **コミットメッセージ**: **必ず日本語**で機能や修正を説明（例: 座席管理機能の状態更新処理を追加）
- **プルリクエスト**: タイトルと説明を**必ず日本語**で記述
- **コメント・レビュー**: **必ず日本語**でディスカッション
- **ブランチ名**: 英語で機能表現（例: `feature/seat-management`）
- **作業記録**: 作業内容や計画はPRコメント内に記述し、**マークダウンファイルは作成しない**
  - ❌ 禁止: CHANGE_SUMMARY.md、IMPLEMENTATION_SUMMARY.md、VERIFICATION_GUIDE.md などの作業記録用マークダウンファイルの作成
  - ✅ 推奨: PRの説明欄やコメント機能を使用して作業内容を記録
